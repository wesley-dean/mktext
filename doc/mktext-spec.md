...
`mktext` is a sourceable Bash library distributed in three generated flavors that
share one public behavior contract and may also be executed directly for
context-free informational operations.
...
The generated runtime/release artifacts are:
...
`dist/mktext.dev.bash` SHALL retain the complete maintained source comments.
`dist/mktext.bash` SHALL retain the established comment-stripped distribution
form.  `dist/mktext.min.bash` SHALL be produced by applying the pinned
Bash-Minifier dependency to `dist/mktext.bash`.

All three generated artifacts SHALL embed the same version, source-revision
timestamp, and source commit defined by ADR-008 and ADR-017.  Each SHALL be
created with mode `0755`, and its first line SHALL be:
...
comments or minimum file size are not specific requirements.  Maintainers MAY
source `src/mktext.bash` directly while developing and testing.

Consumers MAY execute any generated Bash artifact directly for context-free
informational forms.  Direct execution SHALL dispatch the process arguments
through the same public `mktext` function and SHALL terminate the process with the
status returned by that dispatcher.

Automatic process dispatch SHALL occur only when the containing file is being
executed directly and the invocation basename `${0##*/}` is exactly one of:
...
A source copy concatenated into or otherwise executed under a different basename
SHALL define the library without dispatching process arguments or terminating the
containing program.  The decision SHALL use the basename only; directory
components containing the string `mktext` SHALL have no effect.

Invocation through a symbolic link SHALL use the basename visible in `$0`.  A
link named with one of the supported basenames therefore opts into direct mktext
command behavior, while a link such as `adr` to a larger executable containing
...
Normal library operations shall not require external runtime commands.  Build-time
use of bashdeps and Bash-Minifier does not add runtime dependencies to any
published Bash artifact.
...
For every generated Bash artifact, the same informational forms SHALL be available
by direct execution.  For example:
...
When a generated artifact is executed directly, an unknown operation or missing
...
Equivalent direct invocation is available through every generated Bash flavor,
for example:

```text
dist/mktext.bash help
dist/mktext.dev.bash -h
dist/mktext.min.bash --help
```
...
Git metadata was available to the build.  All three flavors from one build SHALL
...
When any generated Bash artifact is executed directly under a supported basename
and the dispatcher returns normally, the process exit status SHALL equal the
corresponding public status above.  Direct execution does not define a second
status vocabulary.
...
The generated artifacts' version, build date, and commit are fixed at build time.
...
metadata, approved build-dependency bytes, template bytes representable by Bash,
context, render delimiter configuration, and Bash behavior relevant to this
specification, `mktext` SHOULD produce the same generated artifact metadata and
the same ordinary rendering results.
...
- direct execution under the supported `mktext`, `mktext.bash`,
  `mktext.dev.bash`, and `mktext.min.bash` invocation basenames for context-free
  informational forms;
- executable mode and interpreter entry point of all three generated Bash
  artifacts;
