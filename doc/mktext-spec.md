...
ADR-008.  Consumers SHOULD source the generated release artifact.  Maintainers
MAY source `src/mktext.bash` directly while developing and testing.
...
The three forms SHALL write identical compact usage text to standard output.
They SHALL emit no diagnostic and return 0 when the complete usage text is
...
The two forms SHALL write identical three-line artifact metadata to standard
output:
...
version information, `mktext` SHALL return status 4.
